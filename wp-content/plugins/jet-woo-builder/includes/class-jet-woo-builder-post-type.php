<?php
/**
 * JetWooBuilder post type template
 */
...
if ( ! class_exists( 'Jet_Woo_Builder_Post_Type' ) ) {
	...
	class Jet_Woo_Builder_Post_Type {
		...
		public function set_body_class( $classes ) {
			...
			$custom_shop_page      = jet_woo_builder_shop_settings()->get( 'custom_shop_page' );

			if ( ( 'yes' === $custom_cart_page && is_cart() ) || ( 'yes' === $custom_checkout_page && is_checkout() ) || ( 'yes' === $custom_myaccount_page && is_account_page() ) ) {
				$classes[] = 'jet-woo-builder-elementor';
			}

			if ( 'yes' === $custom_shop_page && ( is_product_taxonomy() || is_post_type_archive( 'product' ) || is_page( wc_get_page_id( 'shop' ) ) ) ) {
				$classes[] = 'jet-woo-builder-elementor-shop';
			}
